module auth {
    ...
    function getEmail(): Promise<string> {
        const email =
            (<HTMLInputElement>(
                document.getElementById('sign-in-with-password-form-email')
            )).value || '';
    ...
    function getPassword(): Promise<string> {
        const password =
            (<HTMLInputElement>(
                document.getElementById('sign-in-with-password-form-password')
            )).value || '';
        return new Promise<string>((resolve) => {
            resolve(password);
        });
    ...
    const signInWithPasswordForm = document.getElementById(
