module auth {
    ...
    function getEmail(): Promise<string> {
        const email =
            (<HTMLInputElement>(
                document.getElementById('sign-up-with-password-form-email')
            )).value || '';
    ...
    function getPassword(): Promise<string> {
        const password =
            (<HTMLInputElement>(
                document.getElementById('sign-up-with-password-form-password')
            )).value || '';
        return new Promise<string>((resolve) => {
            resolve(password);
        });
    ...
    const signUpWithPasswordForm = document.getElementById(
