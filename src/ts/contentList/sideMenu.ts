module contentList {
    ...
    function renderFolderView(
        folderViewTl: string,
        folderViewForSelectedModalTl: string
    ) {
        const folderListView =
            document.getElementsByClassName('folder-list-view')[0];
        const selectFolderModalView = document.getElementById(
            'select-folder-modal'
        );
        const selectFolderListModalView = document.getElementsByClassName(
            'select-folder-list-view'
        )[0];
    ...
    function addEventToFolderView() {
        ...
        for (let i = 0; i < folderVIew.length; i++) {
            ...
            folderVIew[i].addEventListener('mouseover', function () {
                const folderEditButtonView = document.getElementById(
                    `folder-delete-button-${i}`
                );
    ...
    export function addEventToFolderViewForSelectedModal(contentId: number) {
        const selectFolderModalView = document.getElementsByClassName(
            'folder-view-select-folder-modal'
        );
        for (let i = 0; i < selectFolderModalView.length; i++) {
            selectFolderModalView[i].addEventListener(
                'click',
                function () {
        ...
        // TODO: リファクタリング
        const selectFolderViewCloseButton = document.getElementById(
            'select-folder-view-close-button'
        );
    ...
    function getInputedFolderName(): Promise<string> {
        const inputedFolderName =
            (<HTMLInputElement>(
                document.getElementById('folder-name-text-field')
            )).value || '';
        return new Promise<string>((resolve) => {
    ...
    async function initializeSideMenu() {
        ...
        const folderViewForSelectedModalTl =
            await generateFolderViewForSelectedModal();
